...
//#endif
    ...
    extern uint32_t __VECTOR_RAM[];  //»ńČˇÔÚRAMÖĐµÄÖĐ¶ĎĎňÁż±íĆđĘĽµŘÖ·
    extern uint32_t __VECTOR_TABLE[];//»ńČˇÔÚFLASHÖĐµÄÖĐ¶ĎĎňÁż±íĆđĘĽµŘÖ·
    extern uint32_t __RAM_VECTOR_TABLE_SIZE[];

    uint32_t vector_size = ((uint32_t)__RAM_VECTOR_TABLE_SIZE)/4;//»ńČˇÖĐ¶ĎĎňÁż±í´óĐˇ
	...
	uint32_t * VECTOR_RAM ;  //»ńČˇÔÚRAMÖĐµÄÖĐ¶ĎĎňÁż±íĆđĘĽµŘÖ·
    uint32_t * VECTOR_TABLE;//»ńČˇÔÚFLASHÖĐµÄÖĐ¶ĎĎňÁż±íĆđĘĽµŘÖ·
    uint32_t RAM_VECTOR_TABLE_SIZE = 0x400;

	VECTOR_RAM = (uint32_t *)0x80000000;
	VECTOR_TABLE = (uint32_t *)0x70002000;
	
    uint32_t vector_size = ((uint32_t)RAM_VECTOR_TABLE_SIZE)/4;//»ńČˇÖĐ¶ĎĎňÁż±í´óĐˇ
